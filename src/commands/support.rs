...
use super::{MonitorPolicy, VerifySchedule, VerifyScheduleState};
...
pub(super) fn default_monitor_policy() -> MonitorPolicy {
    MonitorPolicy {
        interval_sec: 300,
        verify_every: 0,
...
pub(super) fn verify_schedule_state_path(archive_dir: &Path) -> PathBuf {
    archive_dir.join("state").join("verify-schedule-state.env")
}

pub(super) fn read_verify_schedule_state(archive_dir: &Path) -> AppResult<VerifyScheduleState> {
...
pub(super) fn write_verify_schedule_state(
    archive_dir: &Path,
    state: &VerifyScheduleState,
) -> AppResult<()> {
    let path = verify_schedule_state_path(archive_dir);
...
pub(super) fn current_daily_slot() -> String {
    Utc::now().format("%Y-%m-%d").to_string()
}

pub(super) fn current_weekly_slot() -> String {
...
pub(super) fn extract_json_object<'a>(raw: &'a str, key: &str) -> Option<&'a str> {
...
pub(super) fn extract_json_value_start<'a>(raw: &'a str, key: &str) -> Option<&'a str> {
    let key_marker = format!("\"{key}\"");
    let key_idx = raw.find(&key_marker)?;
    let after_key = &raw[key_idx + key_marker.len()..];
    let colon_idx = after_key.find(':')?;
...
pub(super) fn extract_json_string_value(raw: &str, key: &str) -> Option<String> {
...
pub(super) fn extract_json_u64_value(raw: &str, key: &str) -> Option<u64> {
    let value = extract_json_value_start(raw, key)?;
...
pub(super) fn extract_json_i32_value(raw: &str, key: &str) -> Option<i32> {
    let value = extract_json_value_start(raw, key)?;
    let bytes = value.as_bytes();
    let mut end = 0usize;
...
pub(super) fn sample_self_usage() -> Option<(u64, f64)> {
...
pub(super) fn append_ops_log_line(archive_dir: &Path, line: &str) -> AppResult<()> {
